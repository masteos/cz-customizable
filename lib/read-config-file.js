...
const readOverloadFile = (CZ_CONFIG_NAME = '.cz-config.local.js') => {
...
const readConfigFile = (CZ_CONFIG_NAME = '.cz-config.js') => {
  ...
  // It seems like find-config still locates config files in the home directory despite the home:false prop.
  const czConfig = findConfig.require(CZ_CONFIG_NAME, { home: false });
